...
describe('ObeoTimeline', () => {
  ...
  it('accepts events into the timeline', () => {
    ...
    const sched = createTimeline<StateTimelineEvent>();
    sched.add({
      state: 'A',
      time: 0,
    });
  ...
  it('can get a previous event', () => {
    ...
    expect(sched.previousEvent(event0)).to.equal(undefined);
    sched.dispose();
  });
  ...
  it('can get the scheduled event at the given time', () => {
    ...
    expect(sched.get(0)).is.eq(undefined);
  ...
  it('puts the second scheduled event after if two events are scheduled at the same time', () => {
    const sched = createTimeline<TimelineNameEvent>();
    sched.add({
      name: 'A',
      time: 0,
    });
  ...
  it('can the next event after the given time', () => {
    const sched = createTimeline<StateTimelineEvent>();
    expect(sched.getAfter(0)).is.eq(undefined);
    sched.add({
      state: 'A',
      time: 0.1,
    });
    ...
    expect(sched.getAfter(3)).is.eq(undefined);
    sched.dispose();
  });

  it('can the event before the event before the given time', () => {
    const sched = createTimeline<StateTimelineEvent>();
    expect(sched.getBefore(0)).is.eq(undefined);
    sched.add({
      state: 'A',
      time: 0.1,
    });
  ...
  //   const timeline = createTimeline<TimelineValueEvent>();
  //   timeline.add({
  //     time: 0,
  //     value: 'a',
  //   });
  ...
  context('Iterators', () => {
    ...
    it('iterates over all items before the given time', () => {
      ...
      sched.forEachBefore(0.3, (event) => {
        expect(typeof event).to.eq('object');
        expect(event.time <= 0.3).to.be.eq(true);
        count++;
      });
    ...
    it('can add items during iteration', () => {
      ...
      const sched = createTimeline<AddedInterface>();
      for (let i = 0; i < 1000; i++) {
        sched.add({ time: i });
      }
      let added = false;
