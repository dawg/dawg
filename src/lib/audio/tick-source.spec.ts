...
describe('ObeoTickSource', () => {
  ...
  context('Ticks', () => {
    ...
    it('ticks increment at the rate of the frequency after started', async () => {
      await runOffline((offline) => {
        ...
        source.start(0);
        offline.onDidTick(() => {
          // console.log(source.getTicksAtTime());
          expect(source.getTicksAtTime()).to.be.closeTo(offline.now(), 0.1);
        });
      }, { duration: 0.5 });
    });
  ...
  //     return Offline(() => {
  //       const source = createTickSource(2);
  ...
  //     const source = createTickSource(3);
  ...
  //     const source = createTickSource(1);
