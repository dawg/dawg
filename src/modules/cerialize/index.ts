...
export function deserializeAs(keyNameOrType : string|Function|INewable<any>|ISerializable|IEnum, keyName? : string) : any {
    ...
    return function (target : any, actualKeyName : string) : any {
        if (!target || !actualKeyName) return;
        var metaDataList : MetaData[] = TypeMap.get(target.constructor) || [];
        var metadata = getMetaData(metaDataList, actualKeyName);
...
function deserializeObjectInto(json : any, type : Function|INewable<any>|ISerializable, instance : any) : any {
    var metadataArray : MetaData[] = TypeMap.get(type);
...
function deserializeArray(source : any[], type : Function|INewable<any>|ISerializable) : any[] {
    var retn : any[] = new Array(source.length);
    for (var i = 0; i < source.length; i++) {
...
function serializeArray(source : any[], type? : Function|ISerializable) : any[] {
...
let serializeKeyTransform: (key: string) => string = null;
let deserializeKeyTransform: (key: string) => string = null;
